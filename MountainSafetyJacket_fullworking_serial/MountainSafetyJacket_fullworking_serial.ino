...
int current = 0; // we'll use this variable to store a number corresponding to which button was pressed
int last = 0;
int setting = 0;
...
{
  Serial.begin(9600);
  pinMode(SENSOR1, INPUT_PULLUP); //set tilt switch sensor as input
...
void loop() 
{
  ...
  if (current == 1 && current != last){
      Serial.print("The soft button was pressed. Setting is now ");
      Serial.println(setting);
      toggleLights(setting);
      delay(30);
      setting += 1; // moved this to below the LED function call so that it has a chance to be zero
    }
...
previous = reading;
// store last button state for comparing next time through the loop
  last = current;
}
